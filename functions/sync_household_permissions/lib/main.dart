  ...
  try {
    ...
    if (teamId.isEmpty) {
      return context.res.json(
          {'success': false, 'message': 'Missing required parameter: teamId'},
          400);
    }
    ...
    if (userIds.isEmpty) {
      return context.res
          .json({'success': false, 'message': 'No members found in team'}, 404);
    }

    // Sync permissions across collections
    final result = await syncPermissionsForCollections(
        context, databases, userIds, teamId);

    return context.res.json({
      'success': true,
      'message':
          'Permissions synced for ${result.totalDocuments} documents in team $teamId',
      'totalDocuments': result.totalDocuments
    });
  } catch (e) {
    context.log('Error syncing household permissions: $e');
    return context.res.json({'success': false, 'message': e.toString()}, 500);
  }
}
...
Future<SyncResult> syncPermissionsForCollections(
    context, Databases databases, List<String> userIds, String teamId) async {
  ...
  for (final collectionName in collectionsToSync) {
    ...
    for (final userId in userIds) {
      final docsUpdated = await syncUserDocuments(
          context, databases, databaseId, collectionName, userId, teamId);
  ...
  context.log(
      'Completed syncing permissions for $totalDocuments documents in team $teamId');
  return SyncResult(totalDocuments);
}

// Sync permissions for a single user's documents in a collection
Future<int> syncUserDocuments(context, Databases databases, String databaseId,
    String collectionName, String userId, String teamId) async {
  int updatedCount = 0;
  ...
  try {
    ...
    context.log(
        'Found ${documents.total} $collectionName documents for user $userId');

    // Update permissions for each document
    for (final doc in documents.documents) {
      try {
        final docId = doc.$id;
        await updateDocumentPermissions(context, databases, databaseId,
            collectionName, docId, userId, teamId);
