      ...
      try {
        ...
        if (userList.total > 0) {
          ...
          if (!hasRecipientReadPermission) {
            context.log(
                'Adding read permission for recipient ${recipientUser.$id} to invitation ${document.$id}');
            ...
            context.log(
                'Successfully added read permission for user ${recipientUser.$id}');
          } else {
            context.log(
                'User ${recipientUser.$id} already has read permission for invitation ${document.$id}');
          }
        } else {
          context.log(
              'No user found with email $recipientEmail, skipping permission update');
        }
      } catch (e) {
