...
class SynchronizedLocationDatabase extends LocationDatabase {
  ...
  void syncDifferences() {
    ...
    final remoteMap = {
      for (final location in remoteChanges) '${location.name}-${location.upc}': location
    };
    final localMap = {
      for (final location in localChanges) '${location.name}-${location.upc}': location
    };
    int changes = 0;

    for (final remoteLocation in remoteChanges) {
      final key = '${remoteLocation.name}-${remoteLocation.upc}';
      if (!localMap.containsKey(key)) {
        local.store(remoteLocation.name, remoteLocation.upc);
        changes++;
        Log.d(
            'Added remote location "${remoteLocation.name}" with UPC "${remoteLocation.upc}" to local database.');
      } else {
        final mergedLocation = localMap[key]!.merge(remoteLocation);
        local.store(mergedLocation.name, mergedLocation.upc);
        remote.store(mergedLocation.name, mergedLocation.upc);
        changes++;
        Log.d(
            'Merged remote location "${remoteLocation.name}" with UPC "${remoteLocation.upc}" with local database.');
      }
    }

    for (final localLocation in localChanges) {
      final key = '${localLocation.name}-${localLocation.upc}';
      if (!remoteMap.containsKey(key)) {
        remote.store(localLocation.name, localLocation.upc);
        changes++;
        Log.d(
            'Added local location "${localLocation.name}" with UPC "${localLocation.upc}" to remote database.');
      }
    }
  ...
  void synchronize() {
    ...
    for (final key in localMap.keys) {
      if (!remoteMap.containsKey(key)) {
        final localLocation = localMap[key]!;
        remote.store(localLocation.name, localLocation.upc); // Add appropriate quantity if needed
      }
    }
