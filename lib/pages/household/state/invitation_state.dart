...
final invitationsProvider = AsyncNotifierProvider<InvitationState, List<Invitation>>(() => InvitationState());
...
class InvitationState extends AsyncNotifier<List<Invitation>> {
  ...
  bool get canSendInvites => state.whenOrNull(
      data: (_) => true,
      ) ?? false;
  ...
  Future<bool> isUserInvited(String email) async {
    final cloudRepo = await _getCloudRepo();
    if (!(cloudRepo.isMultiUser && cloudRepo.isUserVerified && cloudRepo.loggedIn)) {
      return false;
    }
